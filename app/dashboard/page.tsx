...
import { sendMessage, refreshContactInfo } from '../actions'; // We will create refreshContactInfo in Part 2
...
export default function Dashboard() {
  ...
  // 2. Load Messages & Auto-Fetch Profile Info
  useEffect(() => {
    ...
    // Trigger Profile Refresh if missing (Fixes "User 17841...")
    ...
    if (currentChat && !currentChat.username) {
        refreshContactInfo(currentChat.instagram_user_id).then(() => {
            ...
            supabase.from('conversations').select('*').order('last_message_at', { ascending: false }).then(({data}) => setChats(data || []));
        });
    }
            ...
            >
              ...
              <div className="relative">
                <Image
                  ...
                  unoptimized={!!chat.profile_pic_url && chat.profile_pic_url.startsWith('http')}
                />
                {/* Online Dot (Fake for now) */}
      ...
      {activeChat ? (
        <div className="flex-1 flex flex-col bg-white">
          ...
          <div className="h-16 border-b flex items-center justify-between px-6 bg-white shadow-sm z-10">
            <div className="flex items-center gap-3">
              <Image
                ...
                alt={activeChatDetails?.username || "Avatar"}
                width={40}
                height={40}
                className="w-10 h-10 rounded-full object-cover"
                unoptimized={!!activeChatDetails?.profile_pic_url && activeChatDetails.profile_pic_url.startsWith('http')}
              />
              <div>
            ...
            {messages.map((m, i) => (
              <div key={m.id} className={`flex ${m.is_from_me ? 'justify-end' : 'justify-start'} group`}>
                ...
                <div className={`max-w-[70%] p-3 rounded-2xl shadow-sm text-sm leading-relaxed relative ${
                  ...
                  {/* Time Tooltip */}
                  <span className={`text-[10px] absolute -bottom-5 ${m.is_from_me ? 'right-0' : 'left-0'} text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity`}>
          ...
          <div className="p-4 bg-white border-t">
            <div className="flex items-center gap-2 bg-gray-100 rounded-xl px-4 py-2 border border-gray-200 focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-300 transition-all">
